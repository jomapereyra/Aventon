...
class Viaje{
	...
	public function hay_viaje($usuario){
		$registro=$this->db->query("SELECT * FROM viaje WHERE viaje.id_usuario<>$usuario")->rowCount();
		return $registro > 0;
	}

	public function existen_creados($id_usuario,$fecha_actual){
		$registro=$this->db->query("SELECT * FROM viaje WHERE viaje.id_usuario=$id_usuario AND viaje.fecha_salida > $fecha_actual")->rowCount();
		return $registro > 0;
	}
